...
//#define	ASPI_QPI_MODE
#define ASPI_CLK_SLOW
...
#ifdef ASPI_QPI_MODE
	#define MODE_IO_X		MODE_IO_X4
#else
	#define MODE_IO_X		MODE_IO_X1
#endif
...
//LOG_INFO("write data reg:%08x data:%08x\n ",regs->data_reg, link->data);

		}
		aspi_write_reg(bank, regs->ctl_reg, CHAIN_LEN(link->data_len) | link->mode);
//LOG_INFO("write ctrl reg:%08x data:%08x\n ",regs->ctl_reg, CHAIN_LEN(link->data_len) | link->mode);
...
//LOG_INFO("write chain reg:%08x data:%08x\n ",ASPI_REG_CHAIN, chain_reg);
...
static int aspi_send_cmd_qpien(struct flash_bank *bank)
{
	struct qspi_io_chain chain[] =  {
		{CHAIN_CMD, SPIFLASH_QPIEN, 1, MODE_IO_X1},
	};
...
{
	struct qspi_io_chain chain[] =  {
		{CHAIN_CMD, SPIFLASH_QPIDI, 1, MODE_IO_X4},
	};
	aspi_setup_chain(bank, chain, 1);
	return aspi_execute_chain(bank);
}
...
static int aspi_send_cmd_data_out(struct flash_bank *bank, uint8_t cmd,
				  uint16_t len, uint32_t data)
{
	/*
	use the CHAIN_ADDR instead of CHAIN_DATA_OUT becasue it does
	not inlucde ASPI_REG_DATA1 in the definition of [CHAIN_DATA_OUT]
	*/
	struct qspi_io_chain chain[] =  {
		{CHAIN_CMD, cmd, 1, MODE_IO_X},
		{CHAIN_ADDR, data, len, MODE_IO_X},
	};
	aspi_setup_chain(bank, chain, 2);
	return aspi_execute_chain(bank);
}
...
static void aspi_reset_controller(struct flash_bank *bank)
{
	...
	#ifdef	ASPI_CLK_SLOW
	/* Slow down fck in controller */
	aspi_write_reg(bank, ASPI_REG_CLOCK, 0x103);
	aspi_write_reg(bank, ASPI_REG_BYTE_COUNT, 4);
	/* check for fck is stable */
	data = aspi_read_reg(bank, ASPI_REG_CLOCK);
	do {
		status = data & SPIFLASH_FCK_STABLE;
	} while (status & SPIFLASH_FCK_STABLE);
	#else
...
static void aspi_setup_default_read_chain(struct flash_bank *bank)
{
	struct qspi_io_chain chain[] =  {
		#ifdef ASPI_QPI_MODE
		{CHAIN_CMD, SPIFLASH_FAST_READ, 1, MODE_IO_X},	// dummy cycle(s)
		#else
		{CHAIN_CMD, SPIFLASH_READ, 1, MODE_IO_X},	// no dummy cycle
		#endif
		{CHAIN_ADDR, 0, 3, MODE_IO_X},
		{CHAIN_DATA_IN, 0, 4, MODE_IO_X},
	};
	/* setup dummy cycle(s) for Fast Read */
	#ifdef ASPI_QPI_MODE
	aspi_send_cmd(bank, SPIFLASH_WRITE_ENABLE);
	aspi_send_cmd_data_out(bank, SPIFLASH_WRITE_DUMMYCYC, 1, 0x08);  // dummy cycle=1
	aspi_send_cmd(bank, SPIFLASH_WRITE_DISABLE);
	aspi_write_reg(bank, ASPI_REG_LAT, 0x820);			 // delay 1-cycle
	#endif
...
static int aspi_erase_sector(struct flash_bank *bank, uint32_t addr)
{
	...
	struct qspi_io_chain chain[] =  {
		{CHAIN_CMD, aspi->flash->erase_cmd, 1, MODE_IO_X},
		{CHAIN_ADDR, addr, 3, MODE_IO_X},
	};
	aspi_setup_chain(bank, chain, 2);
	return aspi_execute_chain(bank);
}
...
{
	struct qspi_io_chain chain[] =  {
		{CHAIN_CMD, SPIFLASH_PAGE_PROGRAM, 1, MODE_IO_X},
		{CHAIN_ADDR, addr, 3, MODE_IO_X},
		{CHAIN_DATA_OUT, 0, len, MODE_IO_X},
	};
...
static int anarion_qspi_flash_erase(struct flash_bank *bank, int start, int end)
{
	...
	/* Probe must happen prior erase for QE bit */
	if (!(aspi->probed)) {
		LOG_ERROR("Flash bank is not probed");
		return ERROR_FAIL;
	}
...
static int anarion_qspi_probe(struct flash_bank *bank)
{
	...
	/* Probe can happen once after POR for QE bit */
	if (aspi->probed) {
		LOG_INFO("Flash bank is already probed");
		aspi_setup_default_read_chain(bank);
		return ERROR_OK;
	}
